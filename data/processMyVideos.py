...
# df = pd.read_pickle('/home/zqr/codes/GolfDB/data/train_split_1.pkl')
yt_video_dir = '/home/zqr/codes/data/show'
...
def preprocess_videos(anno_id, dim=160):
    ...
    path = '/home/zqr/codes/GolfDB/data/videos_processed_{}/'.format(dim)
    if not os.path.exists(path):
        os.mkdir(path)

    if not os.path.isfile(os.path.join(path, "{}.mp4".format(anno_id))):
        print('Processing annotation id {}'.format(anno_id))
        cap = cv2.VideoCapture(os.path.join(yt_video_dir, '{}.mp4'.format(anno_id)))
